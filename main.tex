...
Figure~\ref{fig:architecture} presents the system architecture, generated from Mermaid source (see \texttt{figures/architecture.mmd}). The desktop application, built on Tauri (Rust backend + web frontend), hosts four primary subsystems: the Core Agent, the Voice Pipeline, the Vision Module, and the Avatar Renderer.

% Mermaid diagram: figures/architecture.mmd
% Render with: mmdc -i figures/architecture.mmd -o figures/architecture.pdf
\begin{figure}[htbp]
    \centering
    ...
    \small See \texttt{figures/architecture.mmd} for Mermaid source.\\
    Four subsystems: Core Agent, Voice Pipeline, Vision Module, Avatar Renderer.
    \end{minipage}
    \caption{B.A.D.D.I.E. system architecture showing the four primary subsystems and data flow.}
    \label{fig:architecture}
\end{figure}

Figure~\ref{fig:personality} illustrates the three-layer personality pipeline (Mermaid source: \texttt{figures/personality-pipeline.mmd}):

% Mermaid diagram: figures/personality-pipeline.mmd
\begin{figure}[htbp]
    \centering
    \begin{minipage}{\columnwidth}
    \centering
    \textbf{[Personality Pipeline Diagram]}\\
    \small Three layers: Base Persona $\rightarrow$ Adaptive $\rightarrow$ Memory-Conditioned.
    \end{minipage}
...
Figure~\ref{fig:memory} depicts the typed memory architecture (Mermaid source: \texttt{figures/memory-architecture.mmd}). Following the ENGRAM \cite{arxiv2511.12960} and Mem0 \cite{arxiv2504.19413} paradigms, B.A.D.D.I.E. organizes memory into three typed stores:
...
% Mermaid diagram: figures/memory-architecture.mmd
\begin{figure}[htbp]
    \centering
    \begin{minipage}{\columnwidth}
    \centering
    \textbf{[Memory Architecture Diagram]}\\
    \small Conversation $\rightarrow$ Extraction $\rightarrow$ Typed Stores (Episodic/Semantic/Procedural) $\rightarrow$ Retrieval $\rightarrow$ LLM Context.
    \end{minipage}
...
Figure~\ref{fig:voice} shows the voice interaction sequence (Mermaid source: \texttt{figures/voice-pipeline.mmd}). The voice pipeline implements a fully local speech-to-speech loop:
...
% Mermaid diagram: figures/voice-pipeline.mmd
\begin{figure}[htbp]
    \centering
    \begin{minipage}{\columnwidth}
    \centering
    \textbf{[Voice Pipeline Sequence Diagram]}\\
    \small User $\rightarrow$ VAD $\rightarrow$ Whisper $\rightarrow$ LLM $\rightarrow$ TTS/Avatar (streaming).
    \end{minipage}
